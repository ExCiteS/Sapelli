...
public abstract class SQLRecordStore extends RecordStore
{
	...
	public List<Record> retrieveRecords(RecordsQuery query)
	{
		...
		if(query.isAnySchema())
		{	// create subqueries for each known schema, use recursive call to execute each, collect & return results:
			for(Schema s : getKnownSchemata())
				result.addAll(retrieveRecords(new RecordsQuery(s, query.getOrderBy(), query.isOrderAsc(), query.getLimit(), query.getConstraints())));
		}
		else
		{
			Schema schema = query.getSourceSchema();
			...
			// TODO generate & execute SELECT query + process & return result as a List
