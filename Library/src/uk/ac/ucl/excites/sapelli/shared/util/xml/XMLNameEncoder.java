 ...
 * 
 * This specific file (XMLNameEncoder.java) has been taken from the
 * ModeShape project (http://www.modeshape.org), which is equally licensed
 ...
 * <p>
 * Any UTF-16 unicode character that is not a valid XML name character according to the <a href="http://www.w3.org/TR/REC-xml/#sec-common-syn">World Wide Web
 * Consortium (W3C) Extensible Markup Language (XML) 1.0 (Fourth Edition) Recommendation</a> is escaped as <code>_xHHHH_</code>, where <code>HHHH</code> stands
 * for the four-digit hexadecimal UTF-16 unicode value for the character in the most significant bit first order. For example, the name "Customer_ID" is encoded
 * as "Customer_x0020_ID".
 * </p>
 * <p>
 * Decoding transforms every <code>_xHHHH_</code> encoding sequences back into the UTF-16 character. Note that {@link #decode(String) decoding} can be safely
 * done on any XML name, even if the name does not contain any encoded sequences.
 * </p>
 * 
 * Class taken (with very marginal modifications) from the ModeShape project (http://www.modeshape.org), licensed under the Apache License, Version 2.0.
 * Specific file revision: https://github.com/ModeShape/modeshape/blob/65cd7f2/modeshape-common/src/main/java/org/modeshape/common/text/XmlNameEncoder.java
 * As far as I know this works for both XML v1.0 and v1.1, given that the relevant parts of the specification seem to be the same:
 *  - XML v1.0: http://www.w3.org/TR/xml/#sec-common-syn
 *  - XML v1.1: http://www.w3.org/TR/xml11/#sec-common-syn
 * 
 * @author mstevens
 */
